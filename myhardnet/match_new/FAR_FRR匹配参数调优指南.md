...
| ---: | --- | ---: | --- | --- | --- |
| 1 | `identification.match_score_threshold` | `0.55` | 阈值增大：FAR 降、FRR 升；阈值减小则相反 | 代码合法范围 `[0,1]`；当前分数体系重点观察 `0.45~0.60` | 不建议手工猜单点。先使用完整阈值曲线，再重点检查 `0.50~0.56`；旧模型参考点为 `0.5326`，新模型必须重标定 |
| 2 | `evaluation.target_far` / `target_frr` | `2e-5` / `0.02` | 不改变原始分数，只改变自动选择和报告的操作点 | 按产品目标固定 | 正式实验保持不变，不作为提升算法精度的手段 |
...
| 25 | `max_candidates_for_ransac` | `300` | 增大可保留更多候选，同时增加噪声与耗时 | 建议 `150~500`；`<=0` 表示不截断，不建议正式使用 | `200、300、400` |
| 26 | `orientation_soft_gate` / `orientation_weight` | `false/0.15` | 仅在候选数超过截断上限时生效；开启后更偏向主方向一致候选 | 开关取 `false/true`；权重建议 `0.05~0.25` | 先保持关闭；若频繁触发候选截断，再试 `true` 和权重 `0.10、0.15、0.20` |
| 27 | `ransac_max_iters` / `ransac_confidence` | `3000/0.995` | 主要影响找到稳定模型的概率和耗时，达到饱和后对指标影响较小 | 迭代 `1000~5000`；置信度 `0.99~0.999` | 首轮保持当前值，只在 RANSAC 失败率异常时调整 |

`orientation_consistency_thresh_deg` 当前只用于诊断统计，不参与最终匹配分数。
