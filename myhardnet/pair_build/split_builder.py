"""按 finger_id 划分 train / val。

作用：
- 读取 index 阶段生成的 sample_index.csv。
- 在 finger_id 粒度上随机划分数据集，避免同一个手指同时出现在训练和验证中。
...
from splits import DATASET_SPLITS
...
def _assign_fingers(
    ...
    """按归一化比例给每个 finger_id 分配 train 或 val。"""

    if train_ratio <= 0 or val_ratio <= 0:
        raise ValueError("train_ratio 和 val_ratio 都必须大于 0")
    ...
    total = len(shuffled)
    train_count = int(round(total * normalized_train_ratio))
    if total >= 2:
...
def build_split(config: dict[str, Any], logger: logging.Logger) -> dict[str, Any]:
    ...
    val_ratio = float(get_nested(config, "split", "val_ratio", default=0.2))
    ...
    assignments = _assign_fingers(finger_ids, train_ratio, val_ratio, seed)
    ...
    split_fingers: dict[str, set[str]] = {
        split_name: set() for split_name in DATASET_SPLITS
    }
    for row in rows:
    ...
    for split_name in DATASET_SPLITS:
        summary[split_name]["finger_count"] = len(split_fingers[split_name])
    ...
    logger.info(
        "split 完成 | train=%d | val=%d",
        summary["train"]["image_count"],
        summary["val"]["image_count"],
    )
    return {
